...
const BLOCKS_DIR = path.join(__dirname, 'src/blocks/project/');
...
function pugConnect(blockName) {
  return new Promise((resolve, reject) => {
    fs.appendFile(`src/pug/connect/_blocks.pug`, `\ninclude ../../blocks/project/${blockName}/${blockName}.pug`, err => {
      if (err) {
...
function lessConnect(blockName) {
  return new Promise((resolve, reject) => {
    fs.appendFile(`src/less/style.less`, `\n@import '../blocks/project/${blockName}/${blockName}.less';`, err => {
      if (err) {
